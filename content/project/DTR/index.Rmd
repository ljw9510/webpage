...
[4. Estimation of Optimal Treatment Regimes](#estimation)

[5. Simlation Studies](#simulation)
...
Q- and A-learning are two approaches to estimate $d^{opt}$ satisfying $\mathbb{E}[Y^{*}\big(\mathbf{d}\big)] \le \mathbb{E}[Y^{*}\big(\mathbf{d}^{opt}\big)], \text{ for all } \mathbf{d} \in \mathcal{D}$ which involve recursive algorithms.
...
### Estimation of Optimal Treatment Regimes {#estimation}
